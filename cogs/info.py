...
class Info(commands.Cog):
    ...
    async def help(self, ctx, state: str = None):
        ...
        # если не назначено
            ...
            emb.add_field(name='Бот Sabo', value='Разработчики: @Director#6404, @St1zy3#8330. Информация по командам'
                                                 ' сервера - /help команды. Если будут вопросы или баги связанные'
                                                 ' с ботом - пишите.')
            emb.add_field(name='Что есть у нас в боте?', value='Бот с каждым днем улучшается , но мы расскажем что '
                                                               'есть на данный момент в нашем боте. Пройдем по порядку,'
                                                               ' попробуем объяснить кратко и понятно все по пунктам.'
                                                               ' Приступим.')
            emb.add_field(name='1. Валюта', value='На нашем сервере есть своя валюта "SH", за эту валюту можно купить'
                                                  ' все на этом сервере у чего только есть стоимость, как получать ее'
                                                  ' можете глянуть с помощью команды "/help валюта".')
            emb.add_field(name='2. Куда тратить валюту?', value='Как получать валюту разобрались, а тратить куда?'
                                                                ' Вообще способов не слишком много , но думаем пока '
                                                                'что неплохо, да и со временем их становится все больше'
                                                                '.'
                                                                ' Сейчас все расскажем подробнее.')
            emb.add_field(name='3. Казино', value='У нас есть пару казино игр , в которых можно как и приумножить свой'
                                                  ' баланс , так и потерять полностью, узнать подробнее про игры '
                                                  'казино можно с помощью команды "/help казино".')
            emb.add_field(name='4. Роли', value='Еще один способ того как можно потратить валюту, у нас на'
                                                ' сервере можно создать кастомную роль, естестенно не за бесплатно,'
                                                ' подробнее читайте тут - "/help роли".')
            emb.add_field(name='5. Голосовые каналы.', value='Интересный способ потратить валюту - создать '
                                                             'приватный голосовой канал который можно настраивать'
                                                             ' (в данный момент открывать и закрывать), зачем? Ну там'
                                                             ' с кентами посидеть в компании и т.д, кому интересно '
                                                             '- "/help войс".')
            emb.add_field(name='6. Кейсы', value='Классная тема - Кейсы. Можно купить кейс и открыть его ,'
                                                 ' после открытия кейса вам выпадет какая - либо сумма денег , как и в'
                                                 ' казино можно поднять деньги , а можно и уйти с пустым кошельком.'
                                                 ' Подробнее - "/help кейсы".')
            emb.add_field(name='7. Питомцы', value='Тоже кейсы , но из них можно выбить питомцев с помощью которых'
                                                   ' можно'
                                                   ' получить очень классные эффекты , выбить питомцев довольно'
                                                   ' сложно и дорого, но я думаю это того стоит. Подробнее - "'
                                                   '/help питомцы".')
            emb.add_field(name='Итоги', value='Вроде самое основное вам поведал , вы все же почитайте , не зря же мы '
                                              'все это писали, а вообще спасибо если пользуйтесь нашим ботом :)')
            await ctx.send(embed=emb)
        ...
        # команды
            ...
            emb.add_field(name='/help питомцы', value="Узнать о кейсах с питомцами и о самих питомцах.:leaves:",
                          inline=False)
            await ctx.send(embed=emb)
        ...
        # Роли
            ...
            emb.add_field(name='Цена.',
                          value='Начнем с ценника, стоимость данной услуги составляет 10.000 SH, не дорого ,'
                                ' уделив немного времени серверу , можно спокойно накопить и купить себе ту роль ,'
                                ' которую вы хотите.',
                          inline=False)
        ...
        # Питомцы
            ...
            emb.add_field(name='Какая стоимость данного кейса?', value="Стоимость одного кейса составляет 5.000 SH.",
                          inline=False)
            emb.add_field(name='Как узнать сколько у меня кейсов с питомцами?', value="Это можно сделать командой "
                                                                                      "'/casepets'.",
                          inline=False)
            emb.add_field(name='Как открыть кейс с питомцами?', value='Открыть кейс можно при помощи команды '
                                                                      '"/casepets открыть".',
                          inline=False)
            ...
            emb.add_field(name='1."Волк"', value='Данный питомец прибавляет процент от суммы выйгрыша в казино на 7%.',
                          inline=False)
            emb.add_field(name='Пример:', value='Вы сыграли в казино на 50.000 SH, победили, вам прибавляется на'
                                                ' баланс 50.000 SH которые вы выйграли и сверху 3.500 SH за счет'
                                                ' того что у вас данный питомец.',
                          inline=False)
            emb.add_field(name='2."Лиса"', value='Данный питомец прибавляет 7% SH от времени которое вы находились '
                                                 'в голосовом канале.',
                          inline=False)
            emb.add_field(name='Пример:', value='Вы просидели в голосовом канале 4 часа, после выхода вам начислятся '
                                                '4.800 SH, за счет того что у вас данный питомец , вам сверху'
                                                ' прибавится 336 SH.',
                          inline=False)
            emb.add_field(name='3."Собака"', value='Данный питомец возвращает вам 5 % от произрыша в казино.',
                          inline=False)
            emb.add_field(name='Пример:', value='Вы поставили в казино 5000 SH и проиграли , за счет того что у'
                                                ' вас данный питомец, вам вернется 250 SH. :exclamation: Если ставка'
                                                ' менее 100 SH возврата средств не будет :exclamation: .',
                          inline=False)
            emb.add_field(name='Как узнать какие у меня есть питомцы?', value='/mypets (В разработке)',
                          inline=False)
            emb.add_field(name=':exclamation: Важно :exclamation:', value='Если у вас несколько питомцев ,'
                                                                          ' выбран может быть только один ,'
                                                                          ' выбрать питомца можно командой '
                                                                          '"/selectpet".',
                          inline=False)
